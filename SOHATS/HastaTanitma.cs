...
namespace SOHATS
{
    public partial class HastaTanitma : Form
    {
        string _dosyaNo = "";
        ...
        Label lblDurum; // Alt ortadaki durum mesajı
        ...
        {
            // Form açıldığında boş gelsin veya parametre alabilir
        }
        ...
        private void txtDosyaNo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                using (SqlCommand komut = new SqlCommand("SELECT * FROM hasta WHERE dosyaNo=@no", SQLHelper.Baglanti))
                {
                    ...
                    {
                        // Yeni Kayıt için Hazırlık
                        DialogResult cvp = MessageBox.Show("Kayıt bulunamadı. Yeni kayıt yapılsın mı?", "Yeni Kayıt", MessageBoxButtons.YesNo);
                        if(cvp == DialogResult.Yes)
                        {
                            Temizle(false); // Dosya No kalsın
                            lblDurum.Text = "< YENİ KAYIT >";
        ...
        private void btnKaydet_Click(object sender, EventArgs e)
        {
            ...
            using (SqlCommand komut = new SqlCommand("", SQLHelper.Baglanti))
            {
                ...
                string sql = "";
                // Dosya No veritabanında var mı tekrar kontrol edelim, _dosyaNo değişkeninden ziyade
                // Fakat basitlik adına _dosyaNo doluysa UPDATE diyelim.
                // Ya da "Kaydet" butonu metninden anlayabiliriz veya simple logic:
                
                // NOT: Gerçek projede DosyaNo PRIMARY KEY olduğu için, önce var mı diye bakmak lazım. 
                // Biz burada daha önce Enter ile arattıysak _dosyaNo dolmuştur.
                ...
                lblDurum.Text = "< İŞLEM TAMAMLANDI >";
                MessageBox.Show("Kaydedildi.");
                _dosyaNo = txtDosyaNo.Text; // Artık kayıtlı
                btnKaydet.Text = "Güncelle";
            }
        }
        ...
        private void InitUI()
        {
            ...
            Label l2 = CreateLabel("T.C. Kimlik Numarası", 250, 20);
            txtTC = CreateText(380, 17, 150);

            Label l3 = CreateLabel("Ad", 80, 50);
            txtAd = CreateText(100, 47, 150);

            Label l4 = CreateLabel("Soyadı", 300, 50);
            txtSoyad = CreateText(380, 47, 150);

            // Orta Sol
            Label l5 = CreateLabel("Doğum Yeri", 20, 90); txtDogumYeri = CreateText(100, 87, 150);
            Label l6 = CreateLabel("Baba Adı", 20, 120); txtBaba = CreateText(100, 117, 150);
            Label l7 = CreateLabel("Anne Adı", 20, 150); txtAnne = CreateText(100, 147, 150);
            
            Label lAddr = CreateLabel("Adres", 20, 180); 
            txtAdres = CreateText(100, 180, 430); txtAdres.Multiline = true; txtAdres.Height = 80;

            // Orta Sağ
            Label l8 = CreateLabel("Doğum Tarihi", 300, 90); dateDogumTarihi = new DateTimePicker(); dateDogumTarihi.Location=new Point(380,87); dateDogumTarihi.Format=DateTimePickerFormat.Short; dateDogumTarihi.Width=110; this.Controls.Add(dateDogumTarihi);
            
            Label l9 = CreateLabel("Cinsiyeti", 310, 120); cmbCinsiyet = CreateCombo(380, 117, new string[]{"BAY","BAYAN"});
            Label l10 = CreateLabel("Medeni Hali", 470, 120); cmbMedeni = CreateCombo(540, 117, new string[]{"EVLİ","BEKAR"});
            Label l11 = CreateLabel("Kan Grubu", 300, 150); cmbKan = CreateCombo(380, 147, new string[]{"0 Rh+","0 Rh-","A Rh+","A Rh-"}); 
            ...
            Label l14 = CreateLabel("Kurum Adı", 20, yAlt+60); txtKurumAdi = CreateText(100, yAlt+57, 150);

            // Alt Sağ
            Label l15 = CreateLabel("Yakının Telefonu", 300, yAlt); txtYakinTel = CreateText(400, yAlt-3, 150);
            Label l16 = CreateLabel("Kurum Sicil No", 300, yAlt+30); txtYakinKurumSicil = CreateText(400, yAlt+27, 150);
            Label l17 = CreateLabel("Kurum Adı", 300, yAlt+60); txtYakinKurumAdi = CreateText(400, yAlt+57, 150);
            ...
            lblDurum = new Label(); lblDurum.Text = "< İŞLEM TAMAMLANDI >"; lblDurum.Location = new Point(250, yBtn+10); lblDurum.AutoSize = true; lblDurum.Font=new Font("Arial", 9, FontStyle.Bold);
            this.Controls.Add(lblDurum);

            btnCikis = CreateButton("Çıkış", 500, yBtn); btnCikis.Click += btnCikis_Click;
        }
