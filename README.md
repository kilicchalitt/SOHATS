...
1. **ADO.NET Kullanımı:** Hazır ORM araçları yerine doğrudan ADO.NET tercih edilerek, veritabanı üzerindeki SQL hakimiyeti ve performans yönetimi el ile sağlanmıştır.
2. **Taşınabilir Veritabanı:** Proje, SQL Server kurulumu gerektirmeyen LocalDB mimarisiyle tasarlanmıştır; bu sayede projenin farklı ortamlarda kurulum gerektirmeden çalışması mümkün kılınmıştır.
3. **Hata Yönetimi:** Veritabanı işlemleri sırasında oluşabilecek bağlantı hataları ve veri tutarsızlıkları için kapsamlı "Try-Catch" blokları ve kontrol mekanizmaları uygulanmıştır.
